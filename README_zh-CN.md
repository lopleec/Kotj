...
Kotj 是一款功能完整、界面简洁的原生 Android 备忘录应用。它采用 Material Design 3，借鉴 iOS 备忘录的文件夹、笔记列表和编辑结构，通过 Kotlin 与 Jetpack Compose 提供符合 Android 使用习惯的体验。

从日常速记到图文长笔记，都可以使用富文本、图片、表格和待办记录内容，通过分类、搜索与置顶整理笔记，再按需导出为常用文档格式。日常使用保存在本地，无需注册账号；加密 Google Drive 备份是一项可自行开启的试验性功能。
...
- 使用同一密钥签名的更新可直接覆盖安装，保留本地数据
- Debug 与正式版签名不同，互相切换前请先导出重要笔记
...
- 加粗、斜体、下划线、删除线和文字颜色，支持叠加下划线与删除线
- 正文、标题、多级标题、编号列表、项目符号和原生复选框待办
- 表格、分界线与系统照片选择器图片插入
...
- 浅色、深色及跟随系统主题
- Android 12+ 动态配色
- 可调整笔记排序、日期分组与最近删除保留时长

### Google Drive 备份（试验性、可选）

- 在「设置 → 高级设置（试验性）」中开启；默认关闭，本地使用无需登录 Google 账号
...
- 支持大体积图文备份的可恢复上传、“立即备份”以及切换 Google 账号；取消切换会保留当前连接
...
如果需要在自行构建的版本中使用 Google Drive，请在应用使用的 Google Cloud 项目中创建 Android OAuth 客户端，登记包名 `com.lopleec.kotj` 与自己的签名证书 SHA-1。Android OAuth Client ID 和 Project ID 属于公开应用配置；OAuth 客户端密钥与签名凭据需要保密。

<details>
<summary>构建正式签名版</summary>
...
Kotj 采用 [GNU General Public License v3.0](LICENSE) 开源许可证，完整条款见仓库中的许可证文件。
